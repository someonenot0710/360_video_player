...
#    cdef np.ndarray vh=np.array([[vxh],[vyh],[vzh]],dtype=float)
...
#    probs2=[0.0 for i in range(int(tile_w*2*tile_h))]
#    probs3=[0.0 for i in range(int(tile_w*2*tile_h*2))]
#    pixels=[[0.0 for i in range(3840)] for j in range(1920)]
...
#    tile_sizeh2=tile_sizeh
#    tile_sizew2=int(float(tile_sizew)/2.0)
#    tile_sizeh3=int(float(tile_sizeh/2.0))
#    tile_sizew3=tile_sizew2
    theta=ctheta
    phi=cphi
    #print ctheta, cphi
    ...
    for i in xrange(0, int(fov_sizew)):
        for j in xrange(0, int(fov_sizeh)):
            ...
            #d_square=(i-(fov_sizew/2))*(i-(fov_sizew/2))+(j-(fov_sizeh/2))*(j-(fov_sizeh/2))
            ...
            probs[int(tile)]=1
#            pixels[int(vf)][int(uf)]=1.0
#            tile=floor(float(vf)/float(tile_sizeh))*tile_w+floor(float(uf)/float(tile_sizew))
    '''
    for i in range(1920):
        for j in range(3840):
            tile=floor(float(i)/float(tile_sizeh))*tile_w+floor(float(j)/float(tile_sizew))
            tile2=floor(float(i)/float(tile_sizeh2))*tile_w*2+floor(float(j)/float(tile_sizew2))
            tile3=floor(float(i)/float(tile_sizeh3))*tile_w*2+floor(float(j)/float(tile_sizew3))
            probs[int(tile)]=probs[int(tile)]+pixels[i][j]
            probs2[int(tile2)]=probs2[int(tile2)]+pixels[i][j]
            probs3[int(tile3)]=probs3[int(tile3)]+pixels[i][j]
    for i in range(int(tile_w*tile_h)):
        probs[i]=probs[i]/(tile_sizeh*tile_sizew)
    for i in range(int(tile_w*2*tile_h)):
        probs2[i]=probs2[i]/(tile_sizeh2*tile_sizew2)
    for i in range(int(tile_w*2*tile_h*2)):
        probs3[i]=probs3[i]/(tile_sizeh3*tile_sizew3)
    return probs,probs2,probs3
    '''
    return probs
#for itv in range(30, 601, 30):
#    fw=open('tiles/sal_'+video+'_%05d.csv'%itv,'w')
#    sal = [[0.0 for x in range(3840)] for y in range(1920)]
def gen_prob(yaw, pitch, fov_degreew, fov_degreeh, tile_w, tile_h):
    ...
    return probs
#    probs,probs2,probs3=cal_fovs(yaw, pitch, fov_degreew, fov_degreeh, float(tile_w), float(tile_h), fov_sizew, fov_sizeh)
 #   return probs,probs2,probs3
